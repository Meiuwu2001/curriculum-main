...
function ListaPreparacion() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch("http://localhost:5000/preparacion");
  ...
  const initializeGrid = () => {
    ...
    gridRef.current = new Grid({
      ...
      server: {
        url: "http://localhost:5000/preparacion",
        then: (data) =>
          data.map((curriculum) => [
            curriculum.persona_id,
            curriculum.nombre,
            curriculum.apellidos,
            curriculum.grados_academicos,
  ...
  return (
    <>
      <h1 class="title">Lista de Preparaciones academicas</h1>
      <div
        ref={wrapperRef}
...
export default ListaPreparacion;
